...
public class TreeLoggers {
	...
	/**
	 * Creates a branch from the given logger but doesnt actually logs the given message. The branch is delayed until an actual log.
	 * Any new branch also doesnt commit any messages until the new branch has a message logged.
	 * If no messages are logged the original message is also lost.
	 * 
	 * @param logger
	 * @param type
	 * @param msg
	...
	static public TreeLogger delayedBranch(final TreeLogger logger, final Type type, final String message, final Throwable caught) {		
		return new DelayedBranchTreeLogger( logger, type, message, caught );	}
	
	static class DelayedBranchTreeLogger implements TreeLogger{
		...
		public TreeLogger branch(final Type type0, final String message0, final Throwable caught0) {
			return TreeLoggers.delayedBranch( this, type0, message0, caught0);
		}			
		
		public void log(final Type type0, final String message0, final Throwable caught0) {
			TreeLogger branch = this.getBranch();
			if( null == branch ){
				final Type type = this.getType();
				final String message = this.getMessage();
				final Throwable caught = this.getCaught();
				branch = commitBranch( type, message, caught );
				this.setBranch(branch);
			}
			branch.log(type0, message0, caught0);
		}

		public TreeLogger commitBranch( final Type type, final String message, final Throwable caught ){
			...
			if( logger instanceof DelayedBranchTreeLogger ){
				...
				if( null == logger ){
					logger = delayed.commitBranch( type, message, caught );
			...
			logger = logger.branch( type0, message0, caught0);
	...
	/**
	 * Creates a TreeLogger that pipes any messages to the given logger as well as recording each message to the given collection.
	 * Subsequent branches will also have their messages captured.
	 * @param logger
	 * @param messages
	 * @return
	 */
