...
public class GenerateJavaScriptAST {
  ...
  private class GenerateJavaScriptVisitor extends JVisitor {
     ...
     * 
     * This method attempts to locate binary operations that are in fact object reference tests against null
     * and substitute a more concise but functionally equivalent form.
     * 
     * Because empty strings in javascript evaluate to false which is wrong for a null test the optimisation is aborted
     * if the type of the reference is object or String the optimisation attempt is aborted.
     * 
     ...
     * @return
     */
    public boolean simplifyReferenceNullOrNotNullTests(final JBinaryOperation binaryOperation, final Context context){
    	boolean optimised = false;
    	
    	while( true ){
    		// operator must be either == or !=
    		final JBinaryOperator operator = binaryOperation.getOp();
    		final boolean equals = operator.equals( JBinaryOperator.EQ );
    		final boolean notEquals = operator.equals( JBinaryOperator.NEQ );
    		if( false == equals && false == notEquals ){
    			break;
    		}    		
    		// one of the expressions must be a null literal...
    		final JExpression left = binaryOperation.getLhs();
        	final JExpression right = binaryOperation.getRhs();
        	
        	// stop optimisation if neither left/right are null. 
        	final boolean leftIsNull = left instanceof JNullLiteral; 
        	final boolean rightIsNull = right instanceof JNullLiteral;
        	if( false == leftIsNull && false == rightIsNull ){
        		break;
        	}
        	
        	// the type of the non literal side must be an Object and not of type Object or String.
        	if( false == leftIsNull ){
        		final JType type = left.getType();
        		if( type instanceof JPrimitiveType ){
        			break;
        		}        		
        		if( this.isObjectOrString( type )){
        			break;
        		}
        	}
        	if( false == rightIsNull ){
        		final JType type = right.getType();
        		if( type instanceof JPrimitiveType ){
        			break;
        		}        		
        		if( this.isObjectOrString( type )){
        	...
        	// one side is null and the other is an type thats not String or Object...
        	
        	// throw away the js expression holding the null literal...
            final JsExpression jsRight = (JsExpression) pop(); // rhs
            final JsExpression jsLeft = (JsExpression) pop(); // lhs
    ...
    /**
     * Helper which tests if the type is in fact string or Object...
     * @param type
     * @return
     */
    boolean isObjectOrString( final JType type ){
    	final JProgram program = type.getJProgram();
    	return program.getTypeJavaLangObject() == type || program.getTypeJavaLangString() == type;
    }
     ...
     * @return
     */
    public boolean simplifyNumberZeroOrNotZeroTests(final JBinaryOperation binaryOperation, final Context context){
    	boolean optimised = false;
    	...
    	while( true ){
        	...
        	// stop optimisation if neither left/right are null. 
        	final boolean leftIsZero = this.isZeroLiteral( left ); 
        	final boolean rightIsZero = this.isZeroLiteral( right ); 
        	if( false == leftIsZero && false == rightIsZero ){
        		break;
        	}
        	...
        	if( false == leftIsZero ){
        		final JType type = left.getType();	
        		if( false == this.isNumberReference( type )){
        			break;
        		}
        	}
        	if( false == rightIsZero ){
        		final JType type = right.getType();
        		if( false == this.isNumberReference( type )){
        			break;
        		}
    ...
    /**
     * Helper which tests if the given expression is a zero literal.
     * @param expression
     * @return
     */
    boolean isZeroLiteral( final JExpression expression ){
    	boolean zero = false;
    	
    	while( true ){
    		if( expression instanceof JIntLiteral ){
        		final JIntLiteral literal = (JIntLiteral) expression;
        		zero = literal.getValue() == 0;
        		break;
        	}
    		if( expression instanceof JLongLiteral ){
        		final JLongLiteral literal = (JLongLiteral) expression;
        		zero = literal.getValue() == 0L;
        		break;
        	}
    		break;
    	}
    ...
    /**
     * Helper which tests if the given type is byte, short, int or long.
     * @param type
     * @return
     */
    boolean isNumberReference( final JType type ){
    	final JProgram program = type.getJProgram();
    	
    	return program.getTypePrimitiveByte() == type || program.getTypePrimitiveShort() == type || program.getTypePrimitiveInt() == type || program.getTypePrimitiveLong() == type;
    }
