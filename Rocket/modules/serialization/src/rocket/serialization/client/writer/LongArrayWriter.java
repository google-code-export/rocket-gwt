...
/**
 * Handles the serialization of a long array.
 * 
...
public class LongArrayWriter extends ObjectWriterImpl {
	...
	static public final ObjectWriter instance = new LongArrayWriter();

	protected LongArrayWriter() {
	}

	protected void write0(final Object object, final ObjectOutputStream objectOutputStream) {
		this.writeLongArray((long[]) object, objectOutputStream);
	}

	protected void writeLongArray(final long[] array, final ObjectOutputStream objectOutputStream) {
		final int elementCount = array.length;
		objectOutputStream.writeInt(elementCount);

		for (int i = 0; i < elementCount; i++) {
			objectOutputStream.writeLong(array[i]);
		}
	}
}
