...
/**
 * Handles the serialization of a boolean array.
 * 
...
public class BooleanArrayWriter extends ObjectWriterImpl {
	...
	static public final ObjectWriter instance = new BooleanArrayWriter();

	protected BooleanArrayWriter() {
	}

	protected void write0(final Object object, final ObjectOutputStream objectOutputStream) {
		this.writeBooleanArray((boolean[]) object, objectOutputStream);
	}

	protected void writeBooleanArray(final boolean[] array, final ObjectOutputStream objectOutputStream) {
		final int elementCount = array.length;
		objectOutputStream.writeInt(elementCount);

		for (int i = 0; i < elementCount; i++) {
			objectOutputStream.writeBoolean(array[i]);
		}
	}
}
