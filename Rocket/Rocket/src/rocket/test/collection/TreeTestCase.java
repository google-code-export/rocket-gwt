...
import rocket.client.collection.Tree;
...
public class TreeTestCase extends TestCase {
    ...
    public void testSet0SimpleRootSet() {
        ...
        assertNull("Null should be returned as first is a new key ["
                + firstPath + "]", firstPrevious);
	...
	public void testSet2ASetFollowedByAReplacingSet() {
		...
		TestCase.assertSame("Did not return replaced value", firstValue,
				secondPrevious);
	...
	public void testSet3TwoSetsWithDifferentPaths() {
		...
		final Object secondValue = "secondValue";
		final Object secondPrevious = tree.setValue(secondPath, secondValue);
		assertNull("Null should be returned as second is a new key ["
				+ secondPath + "]", secondPrevious);
	...
	public void testSet5TheThirdSetReplacesTheValueOfTheFirstSetWhichIsAGrandchildOfTheFirst() {
		...
		TestCase.assertSame("Did not return replaced value of " + thirdPath,
				firstValue, thirdPrevious);
		assertEquals("Size after 3 sets", 3, tree.size());
	}
	...
	public void TestHasValue0() {
		...
		assertTrue("testing for recently set[" + firstPath + "]", tree
				.hasValue(firstPath));

		final String secondPath = "/second";
		assertFalse("testing for unset[" + secondPath + "]", tree
				.hasValue(secondPath));
	}
	...
	public void TestHasValue2() {
		...
		assertTrue("testing for recently set[" + secondPath + "]", tree
				.hasValue(secondPath));
	}
	...
	public void TestHasValue4WithManySubPaths() {
		...
		for (int i = 0; i < 10; i++) {
			path = path + i;
			tree.setValue(path, path + "-value");
			assertTrue("recently added Path, path[" + path + "]", tree
					.hasValue(path));
	...
	public void testGet0() {
		...
		assertSame("testing get for recently set[" + firstPath + "]",
				firstValue, tree.getValue(firstPath));
	}
	...
	public void testGet2() {
		...
		assertSame("testing get for recently set[" + secondPath + "]",
	...
	public void testGet6WithManySubPaths() {
		...
		for (int i = 0; i < 10; i++) {
			...
			assertSame("getting recently added Path, path[" + path + "]",
					value, tree.getValue(path));
	...
	public void testRemoveValue1() {
		...
		assertSame("testing remove on recently set[" + firstPath + "]",
				firstValue, tree.removeValue(firstPath));

		assertEquals(1, tree.size());

		tree.removeValue(firstPath);

		assertEquals("size remains 1 after removing the same path twice", 1,
				tree.size());
	}
	...
	public void testRemoveValue2DoesntLoseChildBranches() {
		...
		assertSame("testing remove on recently set[" + firstPath + "]",
				firstValue, tree.removeValue(firstPath));
		assertSame("check that [" + secondPath
				+ "] was not lost after removing its parent [" + firstPath
				+ "]", secondValue, tree.getValue(secondPath));
	}
	...
	public void testRemoveValue3FromGrandchildBranch() {
		...
		assertSame("testing remove on recently set[" + secondPath + "]",
				secondValue, tree.removeValue(secondPath));
		assertSame("Double check parent was not modified[" + firstPath + "]",
				firstValue, tree.getValue(firstPath));
	...
	public void testRemovePath0RemovesAPathThatOnlyMatchesALoneLeaf() {
		...
		assertEquals("Tree after 4 sets and a remove of a branch", 5 - 1, tree
				.size());
	}
	...
	public void testRemovePath1RemovesAPathWIthSeveralSubBranches() {
		...
		assertEquals("Tree after 4 sets and a remove of a branch", 5 - 3, tree
				.size());
	}
	...
	public void testRemovePath3RemovesBranchesWithMatchingPrefix() {
		...
		tree.removePath("/ap");
		assertEquals(
				"Tree after a sets and a remove of several matching children",
				3, tree.size());
	}
	...
	public void testSize2() {
		...
		tree.setValue("/banana", "/yellow");
		assertEquals(
				"Tree after 3 set - tree is still flat no multiple branches.",
				3, tree.size());
	}
	...
	public void testSize3() {
		...
		assertEquals(
				"Tree after 2 set and 1 replace - tree is still flat no multiple branches.",
				2, tree.size());
	}
	...
	public void testPathIterator0OnlyNextingNoRemovesAlsoVerifiesPathsAreReturnedSorted() {
		final String[] paths = new String[] { "/apple/banana/carrot",
				"/apple/banana", "/zebra", "/apple" };
	...
	public void testPathIterator1WithRemoves() {
		final String[] paths = new String[] { "/apple", "/apple/banana",
				"/apple/banana/carrot", "/zebra" };
	...
	public void testValueIterator0NextsAndRemoves() {
		final String[] paths = new String[] { "/apple", "/banana", "/carrot",
				"/zebra" };
		final String[] values = new String[] { "green", "yellow", "orange",
				"black & white" };
