 ...
 *
 * Uses a BlockyGrid to display a life simulation.
 * Rather than repainting each and every block on each iteration only changes are painted via
 * {@link #setLiveCell} and {@link #setDeadCell}.
 *
...
public class Life {
	 ...
	 * @param trueBias The higher the value the more live cells
	 * A good starting number is -1234567890
	 */
	public void createCells(final int trueBias) {
		...
		System.out.println("randomize: " + j + "/" + (rows * columns));
	}
	...
	public int update() {
		...
		for (int y = 0; y < rows; y++) {
			...
			for (int x = 0; x < columns; x++) {
				...
				final boolean bit = processCell(previousBit,
						surroundingLiveCellCount);
				newCells[rowOffset + x] = bit;

				// dont bother to recolour cells that havent changed only repaint those that have changed.
				if (bit != previousBit) {
					if (bit) {
	 ...
	 *
	 * The universe of the Game of Life is an infinite two-dimensional grid of
	 * cells, each of which is either alive or dead. Cells interact with their
	 ...
	 * simultaneously, and the discrete moment at which this happens is called a
	 * tick. The rules continue to be applied repeatedly to create further
	 * generations.
	 *
	 * @param bit
	 ...
	 * @return
	 */
	protected boolean processCell(boolean bit,
			final int surroundingLiveCellCount) {
		while (true) {
			if (bit) {
	...
	public String toString() {
		return super.toString() + ", pixelGrid: " + pixelGrid + ", cells: "
				+ cells + ", liveCellColour: "
				+ ColourHelper.toCssColour(liveCellColour)
				+ ", deadCellColour: "
