 ...
 * <li>Fixed positioning is only supported for inline css properties set via {@link StyleHelper} and not
...
public class InternetExplorer6StyleSupport extends StyleSupport {
    ...
    public InternetExplorer6StyleSupport() {
        ...
        final boolean quirksMode = BrowserHelper.isQuirksMode();
        this.setFixedPositionLeft(new DynamicExpression(
                quirksMode ? StyleSupportConstants.FIXED_POSITION_LEFT_QUIRKSMODE_EXPRESSION
                        : StyleSupportConstants.FIXED_POSITION_LEFT_EXPRESSION));
        this.setFixedPositionRight(new DynamicExpression(
                quirksMode ? StyleSupportConstants.FIXED_POSITION_RIGHT_QUIRKSMODE_EXPRESSION
                        : StyleSupportConstants.FIXED_POSITION_RIGHT_EXPRESSION));
        this.setFixedPositionTop(new DynamicExpression(
                quirksMode ? StyleSupportConstants.FIXED_POSITION_TOP_QUIRKSMODE_EXPRESSION
                        : StyleSupportConstants.FIXED_POSITION_TOP_EXPRESSION));
        this.setFixedPositionBottom(new DynamicExpression(
                quirksMode ? StyleSupportConstants.FIXED_POSITION_BOTTOM_QUIRKSMODE_EXPRESSION
    ...
    protected String getRuleStyleUserSelect(final JavaScriptObject rule) {
        throw new RuntimeException("InternetExplorer does not support the " + StyleConstants.USER_SELECT
                + " css property");
    }
    ...
    /**
     * Sets a Internet Explorer inline style and also includes a number of special cases.
     * <ul>
    ...
    protected void setInlineStylePosition(final JavaScriptObject style, final String position) {
        ...
        while (true) {
            ...
            final String left = this.getInlineStylePropertyValuePossiblyFromDynamicExpression(style,
                    StyleConstants.LEFT, leftDynamicExpression);
            final String right = this.getInlineStylePropertyValuePossiblyFromDynamicExpression(style,
                    StyleConstants.RIGHT, rightDynamicExpression);
            ...
            final String bottom = this.getInlineStylePropertyValuePossiblyFromDynamicExpression(style,
                    StyleConstants.BOTTOM, bottomDynamicExpression);
        ...
        // append rather than prepend, this way the toString() etc junk that GWT's wrapJSO() function adds doesnt result in a broken cssText
    ...
    protected String getInlineStyleLeft(final JavaScriptObject style) {
        return this.getInlineStylePropertyValuePossiblyFromDynamicExpression(style, StyleConstants.LEFT, this
                .getFixedPositionLeft());
    }

    protected void setInlineStyleLeft(final JavaScriptObject style, final String value) {
        this.setInlineStylePropertyValueOrDynamicExpression(style, StyleConstants.LEFT, value, this
                .getFixedPositionLeft());
    }

    protected String getInlineStyleRight(final JavaScriptObject style) {
        return this.getInlineStylePropertyValuePossiblyFromDynamicExpression(style, StyleConstants.RIGHT, this
                .getFixedPositionRight());
    }

    protected void setInlineStyleRight(final JavaScriptObject style, final String value) {
        this.setInlineStylePropertyValueOrDynamicExpression(style, StyleConstants.RIGHT, value, this
                .getFixedPositionRight());
    }

    protected String getInlineStyleTop(final JavaScriptObject style) {
        return this.getInlineStylePropertyValuePossiblyFromDynamicExpression(style, StyleConstants.TOP, this
                .getFixedPositionTop());
    }

    protected void setInlineStyleTop(final JavaScriptObject style, final String value) {
        this.setInlineStylePropertyValueOrDynamicExpression(style, StyleConstants.TOP, value, this
                .getFixedPositionTop());
    }

    protected String getInlineStyleBottom(final JavaScriptObject style) {
        return this.getInlineStylePropertyValuePossiblyFromDynamicExpression(style, StyleConstants.BOTTOM, this
                .getFixedPositionBottom());
    }

    protected void setInlineStyleBottom(final JavaScriptObject style, final String value) {
        this.setInlineStylePropertyValueOrDynamicExpression(style, StyleConstants.BOTTOM, value, this
                .getFixedPositionBottom());
    }

    /**
     * This method is potentially invoked by {@link StyleSupport#getInlineStyleProperty(Element, String)} if the property is user-select.
     * Unlike other browsers InternetExplorer does not use a css property but rather sets or removes event handlers.
     * 
     * @param element
     * @return
     */
    ...
    /**
     * This dynamic expression simply returns 'absolute' but is used primarily as a marker to note that fixed position is active.
     */
    ...
    protected String getInlineStylePropertyValuePossiblyFromDynamicExpression(final JavaScriptObject style,
            final String propertyName, final DynamicExpression dynamicExpression) {
        String value = null;
    ...
    protected void setInlineStylePropertyValueOrDynamicExpression(final JavaScriptObject style,
            final String propertyName, final String propertyValue, final DynamicExpression dynamicExpression) {
        while (true) {
            boolean fixed = false;
            ...
            // fixed position remove an existing propertyName set up the dynamic expression.
    ...
    protected void setDynamicExpression(final JavaScriptObject style, final String propertyName,
            final String propertyValue) {
    ...
    /**
     * There is no need to normalize rules as Internet Explorer already does this when it loads stylesheets.
     */
