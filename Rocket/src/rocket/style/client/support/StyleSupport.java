...
abstract public class StyleSupport {
	...
	/**
	 * Reads the special cssText property from the element's inline style. Even computed style subclasses should not attempt to
	 * read this property using {@link #getCssText(JavaScriptObject)}
	 * @param element
	 * @return
	 */
	...
	protected String getUserSelectFunction(final JavaScriptObject element, final boolean walkHeirarchy ) {
		return getUserSelectFunction0( element, walkHeirarchy );
	}
	...
	native private String getComputed0(final JavaScriptObject element, final String name)/*-{
	 ...
	 while( element0 ){
	 //value = $wnd.getComputedStyle(element0,null).getPropertyValue( name );
	...
	/**
	 * Depending on the type of Style this method should delegate to one of the following methods.
	 * <ul>
	 * <li>{@link #getPropertyNamesFromCssText(JavaScriptObject)} used by computed FireFox, Opera and Safari styles.</li>
	 * <li>{@link #getPropertyNamesByIndex(JavaScriptObject)} used by all inline styles and computed Internet Explorer style.</li> 
	 * </ul> 
	 */
