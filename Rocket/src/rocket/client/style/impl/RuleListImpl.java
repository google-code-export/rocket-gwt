...
/**
 * The standard implementation that provides the capability to manipulate the rules belonging
 * to a styleSheet as a list.
...
public class RuleListImpl extends DomObjectList{
  ...
  native protected void removeRule0(final JavaScriptObject styleSheet, final int index) /*-{
   ...
   /**
    * Normalizing rules is the process that takes a list of rules and expands rules that whose selector contains
    * more than one individual selector. For each selector a rule is duplicated with a single selector and the same original
    * style in the same slot as the original rule.
    */
