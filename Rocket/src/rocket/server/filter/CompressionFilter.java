...
/**
 * This filter detects if the browser supports gzip compressed output substituting a new
 * HttpServletResponse implementation if required.
 *
 * A request attribute is set to stop multiple installs of this filter which may occur if one resource
 * includes another.
 */
public class CompressionFilter implements Filter {
	...
	private static final String ALREADY_INSTALLED = CompressionFilter.class.getName()
			+ ".ALREADY_INSTALLED";
	 ...
	 * @param chain A chain implementation from the container
	 * @throws IOException
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (request.getAttribute(ALREADY_INSTALLED) == null
	...
	class CompressionFilterServletOutputStream extends
			ServletOutputStream {
		/**
		 ...
		 */
		CompressionFilterServletOutputStream(final ServletOutputStream out)
				throws IOException {
			this.out = out;
			reset();
		}
	...
	class CompressionFilterHttpServletResponse extends
		 ...
		 * @throws IOException if an I/O error occurs.
		 */
		public CompressionFilterHttpServletResponse(
				final HttpServletResponse response) throws IOException {
			super(response);
			this.response = response;
			outputStream = new CompressionFilterServletOutputStream(response
					.getOutputStream());
		}
		 ...
		 * @return The writer
		 * @throws IOException if something goes wrong while returning the writer
		 */
