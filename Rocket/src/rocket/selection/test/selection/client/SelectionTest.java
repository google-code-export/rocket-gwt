...
//        selection.setEnabled( true );
    ...
    protected void testIsTextSelectionEnabledWithinDisabledParent(){
        ...
        button.addClickListener( new ClickListener(){
            public void onClick( final Widget ignored ){
                ...
                final boolean wasDisabled = Window.confirm( "Was it impossible to select text the outside the enabled element (element has a white background)?");
                if( false == wasDisabled ){                   
                    Test.fail( "User confirmed that text selection was still possible even though selection had been disabled for the document.");                   
                }

                final boolean wasEnabled = Window.confirm( "Was it possible to select text the inside the enabled element (element has a gray background)?");
                if( false == wasEnabled ){                   
    ...
    protected void testIsSelectionEmpty(){
        ...
        Window.alert( "Try selecting and not selecting text and confirm that the results reported by the IS TEXT SELECTION EMPTY button are correct...");
        SelectionTest.postponeCurrentTest( 60 * 1000 );
    }
  ...
  continueButton.addClickListener( new ClickListener(){
      public void onClick( final Widget ignored ){
          ...
          final boolean passed = Window.confirm( "Did the EXTRACT button work correctly and extract the selection and append it to the bottom of the document ?");
          if( false == passed ){
  ...
  SelectionTest.postponeCurrentTest( 60 * 1000 );
    ...
    protected void testSurroundSelection(){
        ...
        continueButton.addClickListener( new ClickListener(){
            public void onClick( final Widget ignored ){
                ...
                final boolean passed = Window.confirm( "Did the SURROUND button work correctly and make the selected text larger and with a light gray background?");
                if( false == passed ){
    ...
    protected JavaScriptObject findTextNode( final Element element, final String searchText  ){
        ...
        if( textNode == null ){
            throw new RuntimeException("Unable to find a textNode that is a child of element with the text [" + searchText + "], element: " + DOM.getInnerText(element ));
        }
        return textNode;
    }       
