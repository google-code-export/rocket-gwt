 ...
 * 
 * This class includes a number of small changes to expose the accompanying serializer. Scan for ROCKET
 */
class ProxyCreator {
    ...
    private static final String SERIALIZATION_STREAM_READER_INSTANTIATION = ClientSerializationStreamReader.class
            .getName()
            + " streamReader = new " + ClientSerializationStreamReader.class.getName() + "(SERIALIZER);";

    private static final String SERIALIZATION_STREAM_WRITER_INSTANTIATION = ClientSerializationStreamWriter.class
            .getName()
    ...
    /*
     * This method returns the real type name. Currently, it only affects JParameterizedType since their names are not legal Java names.
     */
    ...
    public ProxyCreator(JClassType serviceIntf, SerializableTypeOracle serializableTypeOracle) {
        this.serviceIntf = serviceIntf;
        this.serializableTypeOracle = serializableTypeOracle;

        if (serviceIntf.isInterface() == null)
            throw new RuntimeException("Expecting a service interface, but " + serviceIntf.getQualifiedSourceName()
                    + " is not an interface");
    }
    ...
    public String create(TreeLogger logger, GeneratorContext context) {
        ...
        logger = logger.branch(TreeLogger.SPAM, "Generating RPC Proxy for service interface '"
    ...
    /**
     * THis method inserts the getter which exposes the serializer accompanying the generated proxy.
     * 
     * @param writer
     * 
     * ROCKET
     */
    protected void generateHasSerializerMethod(SourceWriter writer) {
        writer.println("/**");
        writer
                .println("* This method is added by the rocket framework providing a mechanism to get the internal serializer.");
        writer.println("**/");
        writer.println();
        writer.println("public " + Serializer.class.getName() + " getSerializer(){");
        writer.println("  return SERIALIZER;");
        writer.println("}");
        writer.println();
    }

    /*
     * Give a type emit an expression for deserializing that type from a serialization stream.
     */
    ...
    /*
     * Give a type emit an expression for serializing that type into a serialization stream.
     */
    ...
    private void generateAsynchronousProxyMethod(SourceWriter w, JMethod method) {
        ...
        //
        w
                .println("if (!com.google.gwt.user.client.HTTPRequest.asyncPost(getServiceEntryPoint(), streamWriter.toString(), handler))");
     ...
     * @return
     */
    private boolean generateProxyEncode(SourceWriter w, SerializableTypeOracle serializableTypeOracle,
            JClassType serviceIntf, JMethod method) {
        boolean isStale = false;
        String methodName = method.getName();
        JParameter[] params = method.getParameters();
        w.println();
        w.print("private void __" + methodName + "(" + ClientSerializationStreamWriter.class.getName()
                + " streamWriter");
        for (int i = 0; i < params.length; i++) {
            JParameter param = params[i];
        ...
        w.println("streamWriter.writeString(\"" + serializableTypeOracle.getSerializedTypeName(serviceIntf) + "\");");
    ...
    /**
     * Implements the ServiceDefTarget interface to allow clients to switch which back-end service definition we send calls to.
     */
    ...
    /*
     * Determine the name of the object wrapper class to instantiate based on the the type of the primitive.
     */
    ...
    private SourceWriter getSourceWriter(TreeLogger logger, GeneratorContext ctx) {
        PrintWriter printWriter = ctx.tryCreate(logger, getPackageName(), getProxySimpleName());
        if (printWriter == null) {
            return null;
        }

        ClassSourceFileComposerFactory composerFactory = new ClassSourceFileComposerFactory(getPackageName(),
                getProxySimpleName());
        ...
        composerFactory.addImplementedInterface(HasSerializer.class.getName()); // ROCKET make Proxy also implement HasSerializer
    ...
    private void initializeProperties(TreeLogger logger, GeneratorContext context) {
        ...
        try {
            String propVal = propertyOracle.getPropertyValue(logger, Shared.RPC_PROP_ENFORCE_TYPE_VERSIONING);
    ...
    /**
     * This is really a test method that allows us to assert at the start of the create method that we are in fact dealing with a valid
     * service interface. If you are running with assertions diabled, this code should not be called.
     */
    private boolean isValidServiceInterface(TreeLogger logger, GeneratorContext ctx) {
        assert (serviceIntf != null);
        assert (serializableTypeOracle != null);

        ServiceInterfaceValidator siv = new ServiceInterfaceValidator(logger, ctx, serializableTypeOracle, serviceIntf);
