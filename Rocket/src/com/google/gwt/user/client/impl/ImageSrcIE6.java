...
class ImageSrcIE6 {
	...
	private static native void addTop(JavaScriptObject srcImgMap, Element img, String src) /*-{
	 ...
	 img.attachEvent( "onload", finish );
	 img.attachEvent( "onerror", finish );
	 img.attachEvent( "onabort", finish );
	 }-*/;

	/**
	 * Returns the pending src URL of an image, or <code>null</code> if the
	 * image has no pending src URL.
	 */
	...
	/**
	 * Returns the pending parent for the specified URL, or <code>null</code>
	 * if there is no pending parent for the specified URL.
	 */
	...
	/**
	 * Removes a pending parent's pending status, in preparation for changing
	 * its URL to something else.
	 */
