...
public class JavaToJavaScriptCompiler {
	...
	private static void findEntryPoints(TreeLogger logger,
		...
		JMethod bootStrapMethod = program.createMethod(null, "init"
				.toCharArray(), null, program.getTypeVoid(), false, true, true,
				false, false);
		bootStrapMethod.freezeParamTypes();

		for (int i = 0; i < mainClassNames.length; ++i) {
			String mainClassName = mainClassNames[i];
			JReferenceType referenceType = program
					.getFromTypeMap(mainClassName);
			...
			if (mainMethod == null || !mainMethod.isStatic()) {
				...
				if (mainMethod.isAbstract()) {
					logger.log(TreeLogger.ERROR,
							"Entry method 'onModuleLoad' in entry point class '"
									+ mainClassName + "' must not be abstract",
							null);
					throw new UnableToCompleteException();
				}
				...
				if (!mainMethod.isStatic()) {
					...
					// call
					JNewInstance newInstance = new JNewInstance(program, null,
							mainClass);
					qualifier = new JMethodCall(program, null, newInstance,
							noArgCtor);
				}
			}

			// qualifier will be null if onModuleLoad is static
			JMethodCall onModuleLoadCall = new JMethodCall(program, null,
					qualifier, mainMethod);
	...
	public JavaToJavaScriptCompiler(final TreeLogger logger,
			final WebModeCompilerFrontEnd compiler,
			final String[] declEntryPts, boolean obfuscate, boolean prettyNames)
			throws UnableToCompleteException {
		...
		for (int i = 0; i < declEntryPts.length; i++) {
			String[] all = rpo.getAllPossibleRebindAnswers(logger,
					declEntryPts[i]);
		...
		//
		goldenCuds = compiler.getCompilationUnitDeclarations(logger,
				seedTypeNames);
	...
	public String compile(TreeLogger logger, RebindOracle rebindOracle,
		...
		try {
			return this.compile0(logger, rebindOracle, propertyOracle);
	...
	public String compile0(TreeLogger logger, RebindOracle rebindOracle,
			PropertyOracle propertyOracle) throws UnableToCompleteException {
		...
		try {
			...
			do {
				...
				// TODO When upgrading GWT version reapply changes to this class. ROCKET
				//didChange = loggerOptimiser.execute() || didChange;
			...
			GenerateJavaScriptAST.exec(jprogram, jsProgram, obfuscate,
					prettyNames);
	...
	private void checkForErrors(final TreeLogger logger, boolean itemizeErrors)
		...
		for (int iCud = 0; iCud < goldenCuds.length; iCud++) {
			...
			if (result.hasErrors()) {
				...
				TreeLogger branch = logger.branch(TreeLogger.ERROR,
						"Errors in " + String.valueOf(result.getFileName()),
						null);
		...
		if (compilationFailed) {
			logger.log(TreeLogger.ERROR,
					"Cannot proceed due to previous errors", null);
			throw new UnableToCompleteException();
		}
	}
