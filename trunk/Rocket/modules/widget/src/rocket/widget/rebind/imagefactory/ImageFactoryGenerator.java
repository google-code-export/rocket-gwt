...
/**
 * This factory generates ImageFactory implementations and any accompanying
 * server files.
 * 
...
abstract public class ImageFactoryGenerator extends Generator {
	 ...
	 * Image. If any of the these two tests fail an exception is thrown.
	 * 
	 * @param method
	 */
	...
	protected void throwWrongReturnType(final Method method) {
		this.throwException(new ImageFactoryGeneratorException("The method \"" + method.getName() + "\" must return a \""
				+ ImageFactoryConstants.IMAGE_TYPE + "\" or \"" + ImageFactoryConstants.GWT_IMAGE_TYPE + "\" and not \""
	 ...
	 * 
	 * @param type
	 *            The original interface to be implemented
	 * @param newTypeName
	...
	/**
	 * Implements all methods taken from the original interface in the given
	 * concrete type.
	 * 
	 * @param newType
	 * @param methods
	 ...
	 */
	protected Set<String> implementMethods(final NewConcreteType newType, final Set<Method> methods) {
	...
	/**
	 * Implements a single method, using the appropriate url depending on the
	 * supported characteristics of the browser.
	 * 
	 * If a data url may be used then the contents are encoded and this url is
	 * used. For cases where a data url is not appropriate the file is copied to
	 * the public output directory using a filename that is computed from a hash
	 * of its contents.
	 * 
	 * @param newType
	...
	protected void throwExpectedOneImageFilename(final Method method) {
		throwException(new ImageFactoryGeneratorException("Expected to find one \"" + ImageFactoryConstants.IMAGE_FILE
				+ "\" annotation on method: \"" + this.toString(method) + "\"."));
	}

	/**
	 * Retrieves an inputStream which may be used to read the contents of an
	 * image file. If the resource cannot be located an exception is thrown.
	 * 
	 * @param resourceName
	 * @return An inputStream which may be used to read the image in its
	 *         entirety
	 */
	...
	protected Image createImage(final InputStream inputStream, final String resourceName) {
		...
		try {
			...
			throwException(new ImageFactoryGeneratorException("Unable to load image from \"" + resourceName + "\", cause: "
					+ io.getMessage()));
		}
	...
	/**
	 * Instances of this class represent an image resource loaded from the file
	 * system.
	 */
	static class Image {
		/**
	...
	/**
	 * Checks the method for the rocket.client.widget.ImageFactory.location
	 * annotation and if it is present tests if it the value is server.
	 * 
	 * @param method
	 * @return
	 */
	...
	protected void throwInvalidLocationValue(final Method method, final String value) {
		this.throwException(new ImageFactoryGeneratorException("The method " + method + " contains an invalid \""
				+ ImageFactoryConstants.LOCATION + "\" value of \"" + value + "\"."));
	}

	/**
	 * Tests if a server image should be prefetched or lazy loaded upon first
	 * request.
	 * 
	 * @param method
	 * @return
	...
	protected void throwInvalidServerValue(final Method method, final String value) {
		this.throwException(new ImageFactoryGeneratorException("The method " + method + " contains an invalid \""
				+ ImageFactoryConstants.SERVER_REQUEST + "\" value of \"" + value + "\"."));
	}

	/**
	 * Overrides the getPreloadUrls method to return a comma separated list of
	 * urls.
	 * 
	 * @param newType
	 * @param methods
