...
public class CreateProxyTemplatedFile extends TemplatedCodeBlock {
	...
	protected Object getValue0(final String name) {
		Object value = null;
		while (true) {
			if (Constants.CREATE_PROXY_PROXY_CONSTRUCTOR.equals(name)) {
				value = this.getProxyConstructor();
				break;
			}
			if (Constants.CREATE_PROXY_PROXY_TYPE.equals(name)) {
				value = this.getProxyConstructor();
				break;
			}
			if (Constants.CREATE_PROXY_TARGET_BEAN_PARAMETER.equals(name)) {
				value = this.getTargetBeanParameter();
				break;
			}
			if (Constants.CREATE_PROXY_TARGET_BEAN_TYPE.equals(name)) {
