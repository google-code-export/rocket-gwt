...
/**
 * A Tree like structure which contains directories which in turn may have other directories and branches. Deleting a branch also deletes
 * all children and branches underneath it.
 * 
 * The root of the tree is named "" and must always be present. As such it cannot be removed and therefore the tree always has a minimum
 * size of 1. There is no requirement to separate tree levels using a specific character such as '/'
 * 
 * Iterators are provided to walk the tree with views of paths(keys), values or entries. The entries are of course updatable. A flag may be
 * passed when requesting the iterator to control whether or not sub branchse are also visited.
 * 
...
public class Tree {
    ...
    /**
     * REtrieves the size or the total number of branches including the root with this tree.
     * 
     * @return
     */
    public int size() {
    ...
    /**
     * Retrieves the value of teh branch at the given path. If the path does not exist null is returned.
     * 
     * @param path
     * @return
     */
    ...
    /**
     * Sets or replaces the value at the given path returning the previous value in the case of a replace. If a replace occurs the original
    ...
    /**
     * Removes the branch and its children if one is found. If the branch does not exist nothing occurs.
     * 
     * @param path
     */
    ...
    /**
     * Removes a branch value if one exists. If the branch does not exist nothing occurs.
     * 
     * @param path
     * @return
     */
    ...
    /**
     * Returns an iterator which may be used to iterate over current paths for all entries. Corresponding iterators may be fetched to
     * iterate over values and entries.
     * 
     * @return
     */
    public Iterator pathIterator(final String path, final boolean visitSubBranches) {
        return this.createIterator(path, PATH, visitSubBranches);
    }

    public Iterator valueIterator(final String path, final boolean visitSubBranches) {
        return this.createIterator(path, VALUE, visitSubBranches);
    }

    public Iterator entryIterator(final String path, final boolean visitSubBranches) {
        return this.createIterator(path, ENTRY, visitSubBranches);
    }

    /**
     * Factory method which creates an iterator which may be used to transverse this tree. The type parameter controls whether paths/values
     * or entries are returned by the iterator.
     * 
     * @param path
     * @param type
     * @param visitSubBranches
     *            WHen true sub-branches are visited, otherwise when false they are ignored.
     * @return
     */
    protected Iterator createIterator(final String path, final int type, final boolean visitSubBranches) {
    ...
    public String toString() {
        return super.toString() + this.root + ", modificationCounter: " + modificationCounter;
    }
}