...
/**
 * This factory offers a number of options primarily making it possible to automatically benefit from data urls for supporting browsers.
 * In IE all images will be fetched from the server whilst for other browsers images may be created from data urls or fetched from the server
 * depending on the size of the base64 encoded form of the image file.
 * 
 * It does not attempt to do any of the magic found in ImageBundles.
 * 
 * Several annotations may be used on each method to control various behavioural aspects.
 * Some annotations are ignored (rocket.widget.client.ImageFactory.serverRequest) when a image is sourced from a data url.
 * <table>
 * 		<tr>
 ...
 * </table>
 * 
 * The ImageFactory defines 4 separate system properties that contain the maximum length supported for a particular browser.
 * In the case of IE which doesnt support data urls this value is 0.
 * 
 ...
 * <ul>
 * 
 * Doing some testing and googling it seems that firefox and safari dont have any limits on data uris. If you wish to change this
 * overwrite their values by including the appropriate system properties along with the new value.
 * 
