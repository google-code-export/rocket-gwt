...
public class JavaToJavaScriptCompiler {
	...
	private static void findEntryPoints(TreeLogger logger, RebindOracle rebindOracle, String[] mainClassNames, JProgram program)
			throws UnableToCompleteException {
		JMethod bootStrapMethod = program.createMethod(null, "init".toCharArray(), null, program.getTypeVoid(), false, true, true,
				false, false);
		...
		for (int i = 0; i < mainClassNames.length; ++i) {
			...
			if (mainMethod == null || !mainMethod.isStatic()) {
				...
				if (mainMethod == null) {
					logger.log(TreeLogger.ERROR, "Could not find entry method 'onModuleLoad()' method in entry point class '"
							+ mainClassName + "'", null);
					throw new UnableToCompleteException();
				}

				if (mainMethod.isAbstract()) {
					logger.log(TreeLogger.ERROR, "Entry method 'onModuleLoad' in entry point class '" + mainClassName
				...
				if (!mainMethod.isStatic()) {
					...
					if (noArgCtor == null) {
						logger.log(TreeLogger.ERROR, "No default (zero argument) constructor could be found in entry point class '"
								+ mainClassName + "' to qualify a call to non-static entry method 'onModuleLoad'", null);
						throw new UnableToCompleteException();
					}

					// Construct a new instance of the class to qualify the
					// non-static
					// call
	...
	public JavaToJavaScriptCompiler(final TreeLogger logger, final WebModeCompilerFrontEnd compiler, final String[] declEntryPts)
			throws UnableToCompleteException {
	...
	/**
	 * Creates finished JavaScript source code from the specified Java
	 * compilation units.
	 */
	public String compile(TreeLogger logger, RebindOracle rebindOracle) throws UnableToCompleteException {
		return this.compile( logger, rebindOracle, null );
	}
	public String compile(TreeLogger logger, RebindOracle rebindOracle, final PropertyOracle propertyOracle) throws UnableToCompleteException {
		...
		try {
			...
			// Note that all reference types (even nested and local ones) are in
			// the
			...
			do {
				...
				// rewrite non-polymorphic calls as static calls; update all
				// call sites
				...
				// - polymorphic methods based on return types of all
				// implementors
				...
				// prove that any types that have been culled from the main tree
				// are
			...
			// (7) Generate a JavaScript code DOM from the Java type
			// declarations
	...
	private void checkForErrors(final TreeLogger logger, boolean itemizeErrors) throws UnableToCompleteException {
