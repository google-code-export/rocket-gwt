 ...
 * the License.
 */
package rocket.compiler.test.alternatevaluesassignmentoptimiser.compiler;
...
/**
 * This class decorates its super class adding some checks to ensure expected if statements are transformed,
 * and others that shouldnt be are left alone. This class must be used in conjunction with @link AlternateValuesAssignmentOptimiserGwtTestCase
 * @author Miroslav Pokorny
 */
public class AlternateValuesAssignmentOptimiserTest extends AlternateValuesAssignmentOptimiser {
	final static String MODIFIED = "mustChange0,mustChange1,mustChange2";
	final static String UNMODIFIED = "mustNotChange0,mustNotChange1,mustNotChange2,mustNotChange3";
	...
	protected void checkExpectedBinaryOperationsWereModified() {
		...
		if (false == expected.isEmpty()) {
			throw new AssertionError(
					"Several If statements containing variable assignments that should have been made into tenary statement were found unmodified, BinaryOperations: "
							+ expected);
		}
	}
	...
	protected void convertToTenary(final JIfStatement ifStatement, final Context context, final JBinaryOperation thenAssignment, final JBinaryOperation elseAssignment, final TreeLogger logger) {
		...
		if (false == thenAssignment.isAssignment()) {
			throw new AssertionError("The BinaryOperation " + thenAssignment.getSourceInfo()
		...
		while (unmodified.hasNext()) {
			...
			if (sourceForm.contains(string)) {
				throw new AssertionError("The if statement " + ifStatement.getSourceInfo()	+ " should not have been modified, source:\n\t" + sourceForm.replaceAll("\n", "\n\t"));
			}
		}
		...
		super.convertToTenary(ifStatement, context, thenAssignment, elseAssignment, logger);
	}
