...
/**
 * Common base class for any optimiser that attempts to convert if statements
 * into a tenary or conditional statement equivalent.
 * 
...
abstract class TenaryTransformer {
	...
	/**
	 * Visits all if statements passing each one to be processed by the
	 * {@link #visitIfStatement(JIfStatement, Context, TreeLogger)}.
	 * 
	 * @param jprogram
	 * @param logger
	 * @return
		...
		while (true) {
			if( null == node ){
				if( logger.isLoggable( TreeLogger.DEBUG)){
					logger.log( TreeLogger.DEBUG, "Empty statement - not modified.", null );
				}
				break;
			}
	...
	/**
	 * This method clones the given variable reference, returning a new instance
	 * of the same type.
	 * 
	 * @param source
	 * @return
