 ...
 * 
 * ROCKET
 * This class contains two minor changes to cater for a problem when user code attempts to prevent the default action for
 * function keys from occuring. Unfortunately the keyCode must also be cleared. The changes do this after the
 * EventListener/EventPreview instance is called. 
 */
      ...
      if ($wnd.event.returnValue == null) {
        ...
        if (!@com.google.gwt.user.client.DOM::previewEvent(Lcom/google/gwt/user/client/Event;)($wnd.event)) {
          @com.google.gwt.user.client.impl.DOMImplIE6::currentEventTarget = oldEventTarget;
         	...
         	// if DOM.eventPreventDefault was called we need to clear the keyCode so that function keys dont happen. We do this now so the actual keyCode is not lost after DOM.eventPreventDefault is called.
			if( ! $wnd.event.returnValue ){
				$wnd.event.keyCode = 0;
			}	
