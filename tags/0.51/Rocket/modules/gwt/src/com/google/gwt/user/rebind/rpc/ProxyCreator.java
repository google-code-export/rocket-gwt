...
class ProxyCreator {
	...
	/**
	 * Adds a root type for each type that appears in the RemoteService
	 * interface methods.
	 */
	private static void addRemoteServiceRootTypes(TreeLogger logger, TypeOracle typeOracle, SerializableTypeOracleBuilder stob,
			JClassType remoteService) throws NotFoundException {
		logger = logger.branch(TreeLogger.DEBUG, "Analyzing '" + remoteService.getParameterizedQualifiedSourceName()
		...
		for (JMethod method : methods) {
			...
			if (returnType != JPrimitiveType.VOID) {
				TreeLogger returnTypeLogger = methodLogger.branch(TreeLogger.DEBUG, "Return type: "
			...
			if (exs.length > 0) {
				...
				for (JType ex : exs) {
					if (!exceptionClass.isAssignableFrom(ex.isClass())) {
						throwsLogger = throwsLogger.branch(TreeLogger.WARN, "'" + ex.getQualifiedSourceName()
	 ...
	 */
	private static void addRequiredRoots(TreeLogger logger, TypeOracle typeOracle, SerializableTypeOracleBuilder stob)
	...
	public String create(TreeLogger logger, GeneratorContext context) throws UnableToCompleteException {
		...
		if (GenUtil.warnAboutMetadata()) {
			javadocAnnotationDeprecationBranch = logger.branch(TreeLogger.TRACE,
					"Scanning this RemoteService for deprecated annotations; " + "Please see "
							+ RemoteServiceRelativePath.class.getName() + " for more information.", null);
		}
		...
		// ROCKET adds an extra getter which returns the Serializer.
		this.generateHasSerializerMethod(srcWriter);
	 ...
	 */
	private void generateProxyMethod(SourceWriter w, SerializableTypeOracle serializableTypeOracle, JMethod syncMethod,
			JMethod asyncMethod) {
		...
		for (int i = 0; i < asyncParams.length; ++i) {
			...
			if (i < asyncParams.length - 1 && paramType.isPrimitive() == null
					&& !paramType.getQualifiedSourceName().equals(String.class.getCanonicalName())) {
		...
		w.println("boolean " + tossName + " = isStatsAvailable() && stats(\"" + statsMethodExpr + ":requestStart\", timeStat(\""
				+ getProxySimpleName() + "." + syncMethod.getName() + "\", getRequestId()));");
		...
		if (!shouldEnforceTypeVersioning()) {
			w.println(streamWriterName + ".addFlags(" + ClientSerializationStreamReader.class.getName()
		...
		// Encode all of the arguments to the asynchronous method, but exclude
		// the
		...
		w.println(tossName + " = isStatsAvailable() && stats(\"" + statsMethodExpr + ":requestSerialized\", timeStat(\""
				+ getProxySimpleName() + "." + syncMethod.getName() + "\", getRequestId()));");
		...
		w.println(", \"" + getProxySimpleName() + "." + syncMethod.getName() + "\", getRequestId(), " + payloadName + ", "
				+ callbackName + ");");
		w.outdent();
		w.println("}");
	}
			...
			if (isParameterizedType != null) {
				...
				for (int i = 0; i < methods.length; ++i) {
					if (methods[i] == syncMethod) {
						/*
						 * Use the generic version of the method to ensure that
						 * the server can find the method using the erasure of
	...
	private String getRemoteServiceRelativePath(TreeLogger javadocAnnotationDeprecationBranch) {
		...
		if (metaData.length != 0) {
			if (javadocAnnotationDeprecationBranch != null) {
				javadocAnnotationDeprecationBranch.log(TreeLogger.WARN, "Deprecated use of " + ENTRY_POINT_TAG + "; Please use "
	...
	private SourceWriter getSourceWriter(TreeLogger logger, GeneratorContext ctx, JClassType serviceAsync) {
		...
		String[] imports = new String[] { RemoteServiceProxy.class.getCanonicalName(),
				ClientSerializationStreamWriter.class.getCanonicalName(), GWT.class.getCanonicalName(),
				ResponseReader.class.getCanonicalName(), SerializationException.class.getCanonicalName() };
		...
		// ROCKET Need to implement this interface so comet client code can cast
		// and retrieve the serializer.
		composerFactory.addImplementedInterface(HasSerializer.class.getName());
	 ...
	 * it to deserialize incoming objects.
	 * 
	 * @param writer
	 */
	protected void generateHasSerializerMethod(final SourceWriter writer) {
		writer.println("/**");
		writer.println("* This method is added by the rocket framework providing a mechanism to get the internal serializer.");
		writer.println("**/");
		writer.println();
