...
public class CustomInitMethod extends InitMethod{
	...
	public void write( final SourceWriter writer ){
		...
		writer.println("protected void satisfyInit(final Object " + instance + ") throws Exception {");
		writer.indent();

		// cast to type
		final String type = this.getBeanType().getQualifiedSourceName();
	...
	public void setMethodName(final String methodName) {
		...
		if (null == method) {
			throwInitMethodNotFoundException("Unable to find init method [" + methodName + "]");
		}
	...
	protected void throwInitMethodNotFoundException(final String methodName, final String message) {
		throw new InitMethodNotFoundException("The init method [" + methodName + "]" + message + " on "
				+ this.getBeanType().getQualifiedSourceName());
	}

	protected void throwInitMethodNotFoundException(final String message) {
		throw new InitMethodNotFoundException(message + " on " + this.getBeanType().getQualifiedSourceName());
	}
}
