...
/**
 * A powerful easy sortable table to use. The basic idea is to provide value objects one for each row. THe main purpose of the sub-class is to provide a switch statement
 * that returns values from a value object aka the row given an index ( the column).
 * Once the table has been sorted based on the current sort column the sub-class is again queried for the widget that should be present at a particular cell.
 * This allows one to sort based on a particular value but to present another to the client.
 * THe entire redraw lifecycle and selection of what value object appears on what row and so on is managed by the widget.
 *
 ...
 * (ascending/descending) for any sortable column
 * Refer to the bundle test that shows how simple it is to subclass and use this implementation.
 *
...
public abstract class SortableTable extends ZebraFlexTable{
    ...
    protected List sortRows() {
        // create a list ready to be sorted
    ...
    public String toString() {
        return super.toString() + ", columnComparators: " + columnComparators + ", rows: " + rows + ", sortedColumn: " + sortedColumn + ", ascendingSortImageSource["
                + ascendingSortImageSource + "], descendingSortImageSource[" + descendingSortImageSource
                + "]";
    }
