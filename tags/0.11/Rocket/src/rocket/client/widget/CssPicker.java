...
import rocket.client.dom.StyleSheet;
import rocket.client.dom.StyleSheetsCollection;
...
/**
 * This widget presents a series of buttons with the title of available LINKED stylesheets for a
 * web application.
 * For a LINKed stylesheet to be recognized and made a candidate it must have a title, href.
 * Only one LINKed stylesheet should be active with all others disabled.
 *
...
public class CssPicker extends Composite {
	...
	/**
	 * This text is used as the preceeding label for this css picker.
	 * It must be set before an attempt is made to fetch or create the associated widget.
	 */
	...
	protected StyleSheet getStyleSheet(final Button button) {
		...
		final StyleSheet styleSheet = (StyleSheet) this.getMappings().get(
				button);
		if (null == styleSheet) {
	...
	protected void setStyleSheet(final Button button,
		...
		if (mappings.containsKey(button)) {
			ObjectHelper.handleAssertFailure("parameter:button",
					"The parameter:button has already been mapped.");
		}
	...
	public String toString() {
		return super.toString() + ", horizontalPanel: " + horizontalPanel + ", mappings: "
				+ mappings;
	}
}