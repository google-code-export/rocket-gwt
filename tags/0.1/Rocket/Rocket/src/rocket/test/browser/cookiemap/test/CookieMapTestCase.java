 ...
 * the License.
 */
package rocket.test.browser.cookiemap.test;
...
import rocket.client.browser.BrowserConstants;
import rocket.client.browser.BrowserHelper;
import rocket.client.browser.Cookie;
import rocket.client.browser.CookiesMap;
 ...
 * 
 * The tests which are affected by the iterator.next/remove bug/issue below typically have addCheckPoint statements
 * where I have attempted to figure things out.
 * 
...
public class CookieMapTestCase extends GWTTestCase {
	...
	public String getModuleName() {
		return "rocket.test.browser.cookiemap.Test";
	}
	...
	public void testGet0NonExistingCookie() {
		final CookiesMap cookies = new CookiesMap();
	...
	public void testGet3ExistingCookie() {
		...
		try {
			BrowserHelper.setCookie(appleName
					+ BrowserConstants.COOKIE_NAME_VALUE_SEPARATOR
					+ appleValue);
			BrowserHelper.setCookie(bananaName
					+ BrowserConstants.COOKIE_NAME_VALUE_SEPARATOR
					+ bananaValue);
			BrowserHelper.setCookie(carrotName
					+ BrowserConstants.COOKIE_NAME_VALUE_SEPARATOR
					+ carrotValue);
...
//	}
	...
	public void testValuesCollectionIterator2CallingExtraNextAfterIteratorIsExhaustedThrowsException(){
		...
		try {
			...
			try{
				...
				fail( "The CookieMap.value().iterator() should have thrown an exception because it is empty, object: " + object );
			} catch ( final Exception expected ){
	...
	public void testValuesCollectionIterator3NextFailsFastWhenCookieMapIsModified(){
		...
		try {
			...
			try{
				iterator.next();
				fail( "The CookieMap.value().iterator() should have failed fast (throwing an exception) because the backing CookieMap was modified, iterator: " + iterator );
			} catch ( final Exception expected ){				
			}
	...
	public void testEntrySetIterator2CallingExtraNextAfterIteratorIsExhaustedThrowsException(){
		...
		try {
			...
			try{
				final Object object = iterator.next();
				fail( "The CookieMap.entrySet().iterator() should have thrown an exception because it is empty, object: " + object );
			} catch ( final Exception expected ){
	...
	public void testEntrySetIterator3NextFailsFastWhenCookieMapIsModified(){
		...
		try {
			...
			try{
				iterator.next();
				fail( "The CookieMap.entrySet().iterator() should have failed fast (throwing an exception) because the backing CookieMap was modified, iterator: " + iterator );
			} catch ( final Exception expected ){				
			}
	...
	public void testKeySetIterator2CallingExtraNextAfterIteratorIsExhaustedThrowsException(){
		...
		try {
			...
			try{
				final Object object = iterator.next();
				fail( "The CookieMap.keySet().iterator() should have thrown an exception because it is empty, object: " + object );
			} catch ( final Exception expected ){
	...
	public void testKeySetIterator3NextFailsFastWhenCookieMapIsModified(){
		...
		try {
			...
			try{
				iterator.next();
				fail( "The CookieMap.keySet().iterator() should have failed fast (throwing an exception) because the backing CookieMap was modified, iterator: " + iterator );
			} catch ( final Exception expected ){				
			}
