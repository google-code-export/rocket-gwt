...
/**
 * A variety of methods that help detect what browser is running, creating of absolute urls as well as testing if the browser can take data urls containing an embedded image.
 * @author Miroslav Pokorny (mP)
 */
public class BrowserHelper {
	/**
	 * This method takes care of crafting urls for images cehcking if it is
	 * possible to send back the image encoded using a data scheme If not it
	 * will default to returning a http:// url to a servlet on this server.
	 *
	 * @param request
	 * @param imageSource
	 * @return
	 */
	public static String buildImageSrcUrl(final HttpServletRequest request,
	...
	public static boolean isInternetExplorer6(final HttpServletRequest request) {
		...
		return request.getHeader(BrowserConstants.USER_AGENT).indexOf(
	...
	public static boolean isFireFox1x(final HttpServletRequest request) {
		...
		return request.getHeader(BrowserConstants.USER_AGENT).indexOf(
				BrowserConstants.FIREFOX_1_X) != -1;
	}
}