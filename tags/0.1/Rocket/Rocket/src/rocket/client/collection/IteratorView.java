...
/**
 * This iterator provides a fast failsafe view of another collection.
 * It lso provides automatic support for switching based the type of view for the collection. THis is allows
 * a single iterator class implementation for a view over a map requiring only a switch based on the type
 * to fetch the next object when {@link #next} is called.
 *
 * Immediately after creating an instance {@link #syncModificationCounters() } must be called to synchronize modification counters so
 * that the iterator may fail fast at the right time.
 *
...
public abstract class IteratorView implements Iterator {
	...
	public String toString() {
		return super.toString() + ", modificationCounter: " + modificationCounter
				+ ", viewType: " + viewType;
	}
}