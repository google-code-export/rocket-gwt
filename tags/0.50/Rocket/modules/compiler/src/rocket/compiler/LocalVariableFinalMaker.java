...
public class LocalVariableFinalMaker implements JavaCompilationWorker {
	...
	public boolean work(final JProgram jprogram, final TreeLogger logger) {
		...
		final boolean changed = this.findFinalizableLocalVariables(jprogram, branch);
		branch.log(TreeLogger.DEBUG, changed ? "One or more changes were made." : "No changes were committed.", null);
		return changed;
	}

	protected boolean findFinalizableLocalVariables(final JProgram program, final TreeLogger logger) {
		final TreeLogger branch = logger.branch(TreeLogger.DEBUG,
				"Attempting to find all non final local variables and making them final whereever possible.", null);
			...
			public boolean visit(final JBinaryOperation binaryOperation, final Context context) {
				while (true) {
					...
					if (null == localVariable) {
						final TreeLogger logger = this.getLogger();
						if (logger.isLoggable(TreeLogger.DEBUG)) {
							logger.log(TreeLogger.DEBUG, "Local variable \"" + jvariable.getName()
									+ "\" has already been ruled out as finalizable, assignment: \""
					...
					if (variableScope != currentScope) {
						...
						if (logger.isLoggable(TreeLogger.DEBUG)) {
							logger.log(TreeLogger.DEBUG,
									"Cannot be made final because assignment occurs in different scope, assignment: \""
											+ Compiler.getSource(binaryOperation) + "\"", null);
						}
						break;
					}
					...
					if (null != localVariableExpression) {
						...
						if (logger.isLoggable(TreeLogger.DEBUG)) {
							logger.log(TreeLogger.DEBUG,
									"Cannot be finalized because local variable is assigned values more than once, additional assignment: \""
											+ Compiler.getSource(binaryOperation) + "\".", null);
						}
						break;
					}
	...
	static private class LocalVariable {
		...
		public String toString() {
			return super.toString() + ", variable: \"" + this.name + "\", scope: " + this.scope + ", declaration: " + this.declaration
	 ...
	 */
	protected void makeLocalVariableFinal(final JLocalDeclarationStatement declaration, final JExpression valueExpression,
			final JMethod method, final TreeLogger logger) {
		Checker.notNull("parameter:declaration", declaration);
		...
		while (true) {
			...
			if (null != initializer) {
				...
				if (logger.isLoggable(TreeLogger.DEBUG)) {
					logger.log(TreeLogger.DEBUG, "Already has initializer, making variable final \"" + Compiler.getSource(declaration)
							+ "\"", null);
				}
				break;
			}
	 ...
	 * @param logger
	 */
	protected void moveDeclarationToValueAssignment(final JLocalDeclarationStatement declaration, final JExpression valueExpression,
			final JMethod method, final TreeLogger logger) {
		Checker.notNull("parameter:declaration", declaration);
		Checker.notNull("parameter:valueExpression", valueExpression);
		Checker.notNull("parameter:method", method);
		...
		if (false == changed) {
			throw new AssertionError("Failed to replace value with declaration, declaration: \"" + Compiler.getSource(declaration)
					+ "\" and value \"" + Compiler.getSource(valueExpression) + "\".");
		}
	}
	...
	protected void replaceExpressionWithDeclaration(final JLocalDeclarationStatement declaration, final JExpression valueExpression,
		...
		if (logger.isLoggable(TreeLogger.DEBUG)) {
			logger.log(TreeLogger.DEBUG,
					"@JES@Created a new declaration that is final with an initializer replacing the original assignment expression, declaration: "
							+ Compiler.getSource(newDeclaration), null);
		}
	}
}