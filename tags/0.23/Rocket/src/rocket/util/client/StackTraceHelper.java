...
/**
 * This helper exists primarily to assist modifications made to various emulation classes to support querying for StackTraceElements in web
 * mode.
 * 
 * <h3>Emulated Throwable class changes</h3>
 * The emulated Throwable has been changed to save the caller which created the Throwable object. Its Throwable(Emulated).getStackTrace()
 * lazily calls StackTraceHelper.buildStackTrace() to build an array of StackTraceElements given the saved caller.
 * 
...
public class StackTraceHelper {
    ...
    /**
     * Returns as an array the names of all the functions that make up the current callstack.
     * 
     * @return
     */
    ...
    /**
     * Builds and returns an array of function objects for each level of the callstack.
     * 
     * If the javascript host is within some sort of nested recursive stack parts below the repeated stack will be lost.
     * 
     * @return
     */
    native public static JavaScriptObject getCallStackFunctions() /*-{
     var elements = [];
     var i =  0;
     var context = @rocket.util.client.StackTraceHelper::getCallStackFunctions();
    ...
    /**
     * Builds an array of StackTraceElements with each individual element containing the converted functionNames.
     * 
    ...
    public static StackTraceElement[] buildStackTraceElements(final String[] functionNames) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            while (true) {
                ...
                // if $argumentStartIndex was not found the name does not appear to be a class/method compound name.
                ...
                // by chance it appears the functionName had a '_' but no class/method name.(trailing __)
            ...
            // constructors will have the same name as their class... test for this and blank methodName if this is the case.
    ...
    public static String[] getFunctionNames(final JavaScriptObject context) {
        ObjectHelper.checkNotNull("parameter:context", context);

        final String functionNames = StackTraceHelper.getFunctionNames0(context);
        return StringHelper.split(functionNames, ",", true);
    }
