...
public class CalendarTest implements EntryPoint {
	...
	public void onModuleLoad() {
		...
		popup.addMouseEventListener(new MouseEventAdapter() {
			...
			public void onClick(final MouseClickEvent event) {
				final DialogBox dialogBoxWithCalendar = new DialogBox() {
				...
				dialogBoxWithCalendar.setPopupPositionAndShow(new PositionCallback() {
					public void setPosition(final int offsetWidth, final int offsetHeight) {
						dialogBoxWithCalendar.setPopupPosition(x - offsetWidth / 2, y - offsetHeight / 2);
					}
				});

				dialogBoxWithCalendar.setWidget(createMiniCalendar(date, dialogBoxWithCalendar));

				dialogBoxWithCalendar.show();
			}
		});
		rootPanel.add(popup);
	}
	...
	Widget createLargeCalendar() {
		...
		final Calendar calendar = new Calendar() {
			...
			protected Widget createDayTile(final int year, final int month, final int dayOfMonth) {
