...
public class GWTCompiler extends ToolBase {
	 ...
	 * permutation, including cache checking and recording the inputs and
	 * outputs into a {@link Compilation}.
	 */
	private class CompilationRebindOracle extends StandardRebindOracle {
		...
		public CompilationRebindOracle(ArtifactSet generatorArtifacts) {
			super(compilationState, propOracle, module, rules, genDir, generatorResourcesDir, generatorArtifacts);
		}

		/**
		 * Overridden so that we can selectively record inputs and outputs to
		 * derive the cache key for a compilation. Note that the cache gets
		 * invalidated if the propOracle changes state.
		 */
		@Override
		public String rebind(TreeLogger logger, String in) throws UnableToCompleteException {
	...
	private class DistillerRebindPermutationOracle implements RebindPermutationOracle {
		...
		public String[] getAllPossibleRebindAnswers(TreeLogger logger, String requestTypeName) throws UnableToCompleteException {
			...
			for (Iterator<String[]> iter = perms.iterator(); iter.hasNext();) {
				...
				// Create a snapshot of the property values by setting their
				// values
				// in the property oracle. Because my rebindOracle uses the
				// shared
				// generator context (which in turns uses the propOracle), this
				// has the effect we're after. It isn't reentrant, though, so
				// don't
	...
	public void distill(TreeLogger logger, ModuleDef moduleDef) throws UnableToCompleteException {
		...
		// Place generated resources inside the out dir as a sibling to the
		// module
		generatorResourcesDir = new File(outDir, GWT_COMPILER_DIR + File.separator + moduleDef.getName() + File.separator + "generated");

		// Tweak the output directory so that output lives under the module
		// name.
	...
	/**
	 * Ensure the module has at least one entry point (except in validation
	 * mode).
	 */
	private void checkModule(TreeLogger logger) throws UnableToCompleteException {
		if (!jjsOptions.isValidateOnly() && module.getEntryPointTypeNames().length == 0) {
			logger.log(TreeLogger.ERROR, "Module has no entry points defined", null);
			throw new UnableToCompleteException();
		}
	}

	private void compilePermutations(TreeLogger logger, StandardLinkerContext linkerContext, ArtifactSet generatedArtifacts)
		...
		for (Iterator<String[]> iter = perms.iterator(); iter.hasNext(); ++permNumber) {
			...
			// This is the case in validateOnly mode, which doesn't produce
			// output
			if (js == null) {
				continue;
			}
	...
	private void logProperties(TreeLogger logger, Property[] props, String[] values) {
	...
	/**
	 * Attempts to compile with a single permutation of properties. The result
	 ...
	 * </ul>
	 */
	private String realizePermutation(TreeLogger logger, Property[] currentProps, String[] currentValues, int permNumber,
			ArtifactSet generatorArtifacts) throws UnableToCompleteException {
		...
		//
		CompilationRebindOracle rebindOracle = new CompilationRebindOracle(generatorArtifacts);
		...
		// ROCKET modified method signature to also accept propOracle
		return jjs.compile(logger, rebindOracle, propOracle);
	}

	/**
	 * Runs the compiler. If a gui-based TreeLogger is used, this method will
	 * not return until its window is closed by the user.
	 * 
	...
	private boolean run() {
		...
		if (useGuiLogger) {
			...
			DetachedTreeLoggerWindow loggerWindow = DetachedTreeLoggerWindow.getInstance("Build Output for " + moduleName, 800, 600,
					true);
			...
			// Even if the tree logger window is closed, we wait for the
			// compiler
			// to finish.
	 ...
	 * non-cancellable task, in that it will defer thread interruption until it
	 * is done.
	 * 
	 * @param godot
	 *            the thread that is being waited on.
	 */
