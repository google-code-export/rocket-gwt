...
public class NullAndZeroTestsSimplifierGwtTestCase extends GWTTestCase {
	...
	/**
	 * Checks that the local mustBeTrue is actually a boolean rather than a reference.
	 */
	public void testAssignmentToBooleanVariable(){
		final X x = echo( new X() );
		final boolean mustBeTrue = x == null;
		
		assertEquals( false, mustBeTrue );
		
		final String type = this.typeOf( mustBeTrue );
		assertEquals( "boolean", type );
	}
	
	/**
	 * Returns the javascript type for the given so called boolean variable.
	 * @param booleanValue
	 * @return
	 */
	native String typeOf( final boolean booleanValue )/*-{
