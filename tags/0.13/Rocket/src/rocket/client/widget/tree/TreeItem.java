...
/**
 * A TreeItem widget contains both a main widget and child widgets which appear
 * only if the TreeItem is expanded. A control appears next to the main widget
 * and when clicked either expand(shows) the children or collapses (hides) them.
 * 
...
public class TreeItem extends Composite {
	...
	/**
	 * When a TreeItem is alwaysExpanded its collapser/expander widget is hidden
	 * and its child widget panel is always shown.
	 */
	...
	/**
	 * This image allows the user to expand or collapse a tree node by clicking
	 * it.
	 */
	private Image collapserExpanderImage;

	public Image getCollapserExpanderImage() {
		ObjectHelper.checkNotNull("field:collapserExpanderImage",
				collapserExpanderImage);
	...
	public void setCollapserExpanderImage(final Image collapserExpanderImage) {
		ObjectHelper.checkNotNull("parameter:collapserExpanderImage",
				collapserExpanderImage);
		this.collapserExpanderImage = collapserExpanderImage;
	}

	/**
	 * Note that the image source cannot be set until this TreeItem is attached
	 * to a Tree.
	 * 
	 * @return
	 */
	...
	protected void updateCollapserExpanderImageUrl() {
		...
		image.setUrl(this.isExpanded() ? this.getCollapsedImageUrl() : this
				.getExpandedImageUrl());
	}

	protected void updateCollapserExpanderVisibility() {
		this.getCollapserExpanderImage().setVisible(
				this.getChildren().getWidgetCount() > 0);
	}
	...
	public void collapse() {
		final TreeListenerCollection listeners = this.getTree()
				.getTreeListenerCollection();
	...
	/**
	 * This iterator not only visits the widgets belonging to this TreeItem but
	 * also visits the children of all child TreeItems.
	 * 
	 * @param visitDescendants
	 *            when true visits children and descendants, false visits only
	 *            the direct children.
	 * @return
	 */
	public Iterator iterator(final boolean visitDescendants) {
		return visitDescendants ? this.iteratorWhichVisitsOnlyChildren() : this
				.iteratorWhichVisitsDescendants();
	}

	/**
	 * Returns an iterator which visits only the immediate children of this
	 * TreeItem ignoring any children.
	 * 
	 * @return
	 */
	...
	/**
	 * The iterator returned by this method not only visits the children of this
	 * TreeItem but also visits the children of any child TreeItem. If a
	 * TreeItem child is found its children and their children recursively are
	 * visited before the next sibling of the original TreeItem.
	 * 
	 * @return
	 */
		...
		return new Iterator() {
			...
			public void remove() {
				if (!this.hasRemoveIterator()) {
					throw new RuntimeException(GWT.getTypeName(this)
							+ ".remove() called with previous next()");
				}
			...
			Iterator getRemoveIterator() {
				ObjectHelper.checkNotNull("field:removeIterator",
						this.removeIterator);
			...
			void setRemoveIterator(final Iterator removeIterator) {
				ObjectHelper.checkNotNull("parameter:removeIterator",
						removeIterator);
	...
	/**
	 * The parent TreeItem that this widget is added too. This will not be null
	 * unless this treeItem is the root node, in which case the tree property
	 * will be set.
	 */
	...
	/**
	 * If this treeItem is attached to Tree this property will not be null,
